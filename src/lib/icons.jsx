 ...
 * React port of src/icons.js's ic()/icf() string helpers -> an <Icon> component.
 * Path data is static/developer-authored, so dangerouslySetInnerHTML here
 * carries no injection risk - it's the same markup ic()/icf() used to build.
 * Add a key to PATHS to support a new icon.
 * ==========================================================================*/
const PATHS = {
...
export function Icon({ name, filled = false, className = 'ic' }) {
  const d = PATHS[name] || '';
  return filled ? (
    <svg
      className={className}
